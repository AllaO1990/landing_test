...
export class ChartStore extends ComponentStore<ChartState> {
  public readonly candles$: Observable<any[] | null> = this.select(
    (state: ChartState) => state.candles
  );

  public readonly consolidationZones$: Observable<ActiveZone[] | null> =
    this.select((state: ChartState) => state.consolidationZones);
  ...
  public updateCandles = this.updater(
    (state: ChartState, data: { candles: any; index: number }) => {
  ...
  public readonly loadConsolidationZonesV2 = this.effect(
    (stream$: Observable<Idea | null>) => {
      return stream$.pipe(
