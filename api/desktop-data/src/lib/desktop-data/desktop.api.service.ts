...
export class DesktopApiService extends DesktopService {
	...
	public getActiveStock(list: (string | number)[]): Observable<StockPrice<WithLastPrice>> {
		return this._http
			...
			.pipe(
				...
				filter((response: Response<StockPrice<WithLastPrice>>) => response && response.message === ResponseMessage.success),
	...
	editAccountPortfolio(data: AccountPortfolio): Observable<Response<AccountPortfolio>> {
	...
	getIdea(id: StockId): Observable<Response<StockPosition | null>> {
		return this._http.get<Response<StockPosition | null>>(`${this.host}/v1/ideas/${id}`);
	}
