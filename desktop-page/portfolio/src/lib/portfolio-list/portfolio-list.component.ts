...
export class PortfolioListComponent implements AfterViewInit {
	...
	ngAfterViewInit(): void {
		const params$: Observable<Params> = combineLatest([
			...
			debounceTime(0),
			map((params: [AccountBroker, AccountCurrency, AccountPortfolio, AccountType, AccountStrategy, AccountCurrency]) => ({
	...
	async openDialogBalance(event: Event): Promise<void> {
		...
		this._openDialog(this.#dialogBalanceComponent as PolymorpheusComponent<BalanceComponent>, {}, 'Депозит').subscribe();
	}
